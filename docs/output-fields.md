...
| `Travel distance` | Sum of path lengths from base to each lateral root tip | length | > 0 |
| `Total minimal distance` | Sum of Euclidean distances (straight-line) to all tips | length | > 0 |
| `Tortuosity (Material/Total Distance Ratio)` | Total root length / Total minimal distance | ratio | ≥ 1.0 |
...
| `PR minimal length` | Euclidean distance from hypocotyl to primary root tip | length | > 0 |
...
| `LR density` | Number of lateral roots / PR length | 1/length | ≥ 0 |
...
| `Mean LR minimal distances` | Average Euclidean distance from LR tip to insertion point | length | ≥ 0 |
| `Median LR minimal distances` | Median Euclidean distance from LR tip to insertion point | length | ≥ 0 |
| `Sum LR minimal distances` | Sum of Euclidean distances for all LRs | length | ≥ 0 |
...
| `LR minimal distance` | Array of Euclidean distances for each LR | length[] | ≥ 0 |
...
| `Branched zone density` | LR count / Branched zone length | 1/length | ≥ 0 |
...
These fields are only present when **"Include 3D Pareto analysis"** is enabled. They extend the 2D analysis to include path coverage as a third objective.

### Actual Tree Metrics

| Field | Description | Units | Range |
|-------|-------------|-------|-------|
| `Path tortuosity` | Sum of tortuosity values for all root paths | ratio | ≥ 1.0 |
...
**Note**: `epsilon_3d = max(epsilon_3d_material, epsilon_3d_transport, epsilon_3d_coverage)`
...
| Field | Description | Units | Range |
|-------|-------------|-------|-------|
| `Barycentre x displacement` | Vertical distance from hypocotyl to convex hull center | length | any |
| `Barycentre y displacement` | Horizontal distance from hypocotyl to convex hull center | length | any |
